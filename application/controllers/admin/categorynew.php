...
class Categorynew extends CI_Controller {
	...
	public function get_datas(){//
		for($i=1;$i<10;$i++){//http://www.ybdu.com/modules/article/index.php?class=2&page=2
			$url = 'http://all.hengyan.com/35/0_0_0_0_0_0_0_0_0_'.$i.'.aspx';//http://www.ybdu.com/modules/article/index.php?class=1&page=1
			$this->get_data($url);
		}
	...
	public function get_data($url){ 
		...
		foreach($datas as $k=>$v){
			...
			$data['rht'] = $data['lft']+1;
			$data['lang'] = $this->editlang;
			$ishas = $this->Data_model->getSingle(array('name'=>$data['name']),'lee_category');
	...
	function get_book_list($url){
		...
		$reg = '/<div class="list">(.*)<\/div>/isU';
		preg_match($reg,$res,$con);
		$regul = '/<ul><li class="num">(.*)<\/ul>/isU';
		preg_match_all($regul,$con[1],$conul);
		$res = $conul[1][0];
		if(!empty($conul[1])){
			//$number = count($con[0]);
			foreach($conul[1] as $k=>$v){
				$reg_bookname = '/<li class="bookname"><a href="(.*)" target="_blank"(.*)>(.*)<\/a>/isU';
				$reg_bookmaker = '/<li class="author">(.*)<\/li>/isU';
				$reg_booklastupdatetime = '/<li class="update">(.*)<\/li>/isU';
				$reg_size = '/<li class="length">(.*)<\/li>/isU';
				$reg_count = '/<li class="click">(.*)<\/li>/isU';
				...
				$book['bookurl'] = $book[1];
				$book['bookname'] = str_replace('','',$book[3]);
				$book['size'] = $book_size[1];
				$book['uid'] = 1;
				$book['count'] = $reg_count[1];
				$books[] = $book;
			}
			return $books;
		}
	...
	function get_book(){
		...
		//$mid = $maxid['mid'];
		$book_list = $this->Data_model->getData("id >=12831 and is_wj=0 ",'id',0,0,'lee_category');
		$furl = 'http://www.hengyan.com/';
		$i = 0;
	...
	function get_img(){
			...
			//echo $mid;exit;
			$book_list = $this->Data_model->getData("id >=12831",'id',0,0,'lee_category');
			...
			foreach($book_list as $k=>$v){
				...
				if(!empty($book_contant_nr)){
					...
					//$reg_count = '/.*.*)..<\/p>/isU';
					$reg_img = '/<div class="huobg".*<img src="(.*)" alt=.*<\/a><\/div>/isU';
	...
	function get_contant($url,$furl,$cid,$bookname,$is_wj=0,$zj,$is_first,$book_contant,$parentid){
		//echo $is_first;exit;
			$reg_description = '/class="intro ih1">(.*)<\/p>/isU';
			preg_match($reg_description,$book_contant,$book_descriptions);//

			$reg_book_contant = '/var bookid=(.*)\;/isU';
			preg_match($reg_book_contant,$book_contant,$book_contant_list);//span class="colhui">9. </span>
		
			$booklisturl = "http://www.hengyan.com/dir/".$book_contant_list[1].'.aspx';
			$booklistcontant = $this->curlGet($booklisturl);
			$reg_list = '/<div class="chapter">(.*)<\/div>/isU';
			preg_match($reg_list,$booklistcontant,$book_contant_list_nr_all);//
			$reg_book_contant_list = '/<li>(.*)<\/li>/isU';
			preg_match_all($reg_book_contant_list,$book_contant_list_nr_all[1],$book_contant_list_nr);//
		...
		$path = 'data/atricle/';
			foreach($book_contant_list_nr[1] as $kk=>$vv){
				...
				if(($kk+1)>$mid){
					...
					$data['title'] = $book_contant_list_url[3];
					$contant = $this->curlGet($furl.$book_contant_list_url[1]);
					$reg_contant = '/class="content"(.*)<\/div>/isU';
			...
			//
					...
					$data['status'] = 1;
					$data['lang'] = 'zh_cn';
					$data['count'] = round($book_reg_count/3);
