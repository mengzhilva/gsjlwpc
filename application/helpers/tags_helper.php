...
function get_gsjl($limit){
	$CI =& get_instance();
	$data=$CI->Cache_model->get_gsjl($limit);
	return $data;
}

function x6cms_loadCategoryById($id){
	$CI =& get_instance();
	$data=$CI->Cache_model->loadCategoryById($id);
	return $data;
}
function x6cms_loadCategoryByParent($pid,$limit){
	$CI =& get_instance();
	$data=$CI->Cache_model->loadCategoryByParent($pid,$limit);
	return $data;
}
function x6cms_loadCategoryByDowns($limit){
	$CI =& get_instance();
	$data=$CI->Cache_model->loadCategoryByDowns($limit);
	return $data;
}
...
function x6cms_index_tj($id,$categoryid){
	$CI =& get_instance();
	$data=$CI->Cache_model->load_index_tj($id,$categoryid);
	return $data;
}
